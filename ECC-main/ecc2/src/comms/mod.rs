...
/// Receive unread messages for a specific session.
pub fn receive(db: &StateStore, session_id: &str, limit: usize) -> Result<Vec<(i64, MessageType)>> {
    let raw_messages = db.list_messages_for_session(session_id, limit)?;
    let mut parsed = Vec::new();
    for msg in raw_messages {
        if !msg.read {
            if let Some(parsed_msg) = parse(&msg.content) {
                parsed.push((msg.id, parsed_msg));
            }
        }
    }
    Ok(parsed)
...
pub fn mark_as_read(db: &StateStore, session_id: &str) -> Result<usize> {
