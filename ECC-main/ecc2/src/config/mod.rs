...
impl Config {
    ...
    pub fn load() -> Result<Self> {
        ...
        let project_paths = std::env::current_dir()
            ...
            .unwrap_or_default();
        let mut config = Self::load_from_paths(&global_paths, &project_paths)?;

        if let Ok(db_path) = std::env::var("ECC_DB_PATH") {
            config.db_path = PathBuf::from(db_path);
        }
        if let Ok(worktree_root) = std::env::var("ECC_WORKTREE_ROOT") {
            config.worktree_root = PathBuf::from(worktree_root);
        }
        if let Ok(default_agent) = std::env::var("ECC_DEFAULT_AGENT") {
