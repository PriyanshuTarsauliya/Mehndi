...
impl StateStore {
    ...
    pub fn aggregate_session_metrics(&self) -> Result<SessionMetrics> {
        let mut stmt = self.conn.prepare(
            "SELECT 
                SUM(input_tokens), 
                SUM(output_tokens), 
        ...
        let metrics = stmt.query_row([], |row| {
            Ok(SessionMetrics {
                input_tokens: row.get::<_, Option<u64>>(0)?.unwrap_or_default(),
                output_tokens: row.get::<_, Option<u64>>(1)?.unwrap_or_default(),
                tokens_used: row.get::<_, Option<u64>>(2)?.unwrap_or_default(),
                tool_calls: row.get::<_, Option<u64>>(3)?.unwrap_or_default(),
                files_changed: row.get::<_, Option<u32>>(4)?.unwrap_or_default(),
                duration_secs: row.get::<_, Option<u64>>(5)?.unwrap_or_default(),
                cost_usd: row.get::<_, Option<f64>>(6)?.unwrap_or_default(),
            })
        })?;
