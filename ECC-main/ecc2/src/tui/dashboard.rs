...
impl Dashboard {
    ...
    fn render_status_bar(&self, frame: &mut Frame, area: Rect) {
        ...
        } else if let Some(input) = self.new_session_input.as_ref() {
            format!(" session>{input}_ | [Enter] queue [Esc] cancel |")
        ...
        let text = if self.active_completion_popup.is_some()
            ...
            || self.new_session_input.is_some()
    ...
    pub async fn new_session(&mut self) {
        if self.search_input.is_some() || self.spawn_input.is_some() {
            self.set_operator_note(
                "finish current input before opening new session prompt".to_string(),
            );
            return;
        }
        self.new_session_input = Some(String::new());
        self.set_operator_note("type task for new session and press Enter".to_string());
    }

    pub async fn submit_new_session_prompt(&mut self) {
        ...
        let Some(task) = self.new_session_input.take() else {
            return;
        };
        if task.trim().is_empty() {
            self.new_session_input = Some(task);
            self.set_operator_note("new session task cannot be empty".to_string());
            return;
        }
    ...
    pub fn push_input_char(&mut self, ch: char) {
        if let Some(input) = self.spawn_input.as_mut() {
            input.push(ch);
        } else if let Some(input) = self.new_session_input.as_mut() {
            input.push(ch);
    ...
    pub fn cancel_input(&mut self) {
        ...
        } else if self.new_session_input.take().is_some() {
            self.set_operator_note("new session input cancelled".to_string());
    ...
    pub async fn submit_input(&mut self) {
        ...
        } else if self.new_session_input.is_some() {
            self.submit_new_session_prompt().await;
    ...
    fn selected_session_metrics_text(&self) -> String {
        if let Some(session) = self.sessions.get(self.selected_session) {
            ...
            if let Ok(aggregate) = self.db.aggregate_session_metrics() {
                lines.push(format!(
                    "Global Metrics | Tokens {} ({} in / {} out) | Calls {} | Files {} | Duration {} | Cost {}",
                    format_token_count(aggregate.tokens_used),
                    format_token_count(aggregate.input_tokens),
                    format_token_count(aggregate.output_tokens),
                    aggregate.tool_calls,
                    aggregate.files_changed,
                    format_duration(aggregate.duration_secs),
                    format_currency(aggregate.cost_usd)
                ));
            }
